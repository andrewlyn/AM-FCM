...
WAVEFORMS = ["ricker", "asymmetric"]
NOISE_TYPE = "WGN"
OUTPUT_DIR = SCRIPT_DIR / "results_waveform_robustness_MStd"

def evaluate_pair(frequency_index, frequency, snr_db, repeat_id):
    ...
    for waveform in WAVEFORMS:
        ...
        noisy, _, metadata = ec.simulate(
            ...
            noise_type=NOISE_TYPE, noise_bank=None, rng=rng
        )
        rows.append(
            _one_row(waveform, frequency, snr_db, repeat_id, noisy, metadata)
        )
    return rows
...
def main():
    ...
    print("=" * 78, flush=True)
    print("Waveform robustness: Ricker vs Asymmetric, noise = WGN", flush=True)
    print(f"Frequencies: {freq_use}", flush=True)
    print(f"SNR: {snr_use}", flush=True)
    print(f"Repeats: {n_repeat} per frequency-SNR-waveform", flush=True)
    ...
    with parallel_backend("loky", n_jobs=N_JOBS, inner_max_num_threads=1):
        ...
        )(delayed(evaluate_pair)(*task) for task in tasks)
    ...
    plot_comparison(
        by_snr, WAVEFORMS,
        "Waveform robustness: symmetric Ricker vs asymmetric waveform",
        OUTPUT_DIR / "waveform_robustness_le2_le5_le10.png"
    )
